...
if (isset($_POST['history'])) {
	...
	if ($error === 0 && $err ===0){
		if($img_size > 30000000 ) {
			...
			header("location: uphistory.php?error=$em");
		}else{
			...
			if (in_array($img_ex_lc, $allowed_exs)&&in_array($pdf_ex_lc,$allowed_exss) ) {
				...
				$img_upload_path = 'history/image/'.$new_img_name;
				move_uploaded_file($tmp_name, $img_upload_path); 

				$new_pdf_name= uniqid("PDF-",true).'.'.$pdf_ex_lc;
				$pdf_upload_path = 'history/'.$new_pdf_name;
				move_uploaded_file($pdf_tmp_name, $pdf_upload_path);
				
				$sql ="INSERT INTO history(book_name,author_name,details,image_url,pdf_url) VALUES ('$book_name','$author','$detail','$new_img_name','$new_pdf_name')";
				mysqli_query($conn, $sql);

				header("location: uphistory.php");
