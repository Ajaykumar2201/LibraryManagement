...
if (isset($_POST['general'])) {
	...
	if ($error === 0 && $err ===0){
		if($img_size > 3000000000 ) {
			...
			header("location: upgenfr.php?error=$em");
		}else{
			...
			if (in_array($img_ex_lc, $allowed_exs)&&in_array($pdf_ex_lc,$allowed_exss) ) {
				...
				$img_upload_path = 'general/image/'.$new_img_name;
				move_uploaded_file($tmp_name, $img_upload_path); 

				$new_pdf_name= uniqid("PDF-",true).'.'.$pdf_ex_lc;
				$pdf_upload_path = 'general/'.$new_pdf_name;
				move_uploaded_file($pdf_tmp_name, $pdf_upload_path);
				$sql ="INSERT INTO general(book_name,author_name,details,image_url,pdf_url) VALUES ('$book_name','$author','$detail','$new_img_name','$new_pdf_name')";
				
				mysqli_query($conn, $sql);
				header("location: upgenfr.php");
			}else{
